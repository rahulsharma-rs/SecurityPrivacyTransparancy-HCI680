...
from pypdf import PdfReader  # lightweight PDF text extraction for IRB scope parsing
...
# ---------------- Config ----------------
# Simple set of direct identifiers to flag "minimum necessary" / de-ID issues (HIPAA).
DIRECT_IDENTIFIERS = {
...
# Regex patterns to pull structured fields from an IRB PDF (free-text form).
IRB_FIELDS = {
...
# Default retrieval prompts to fetch short policy excerpts from each corpus (for citations).
...
def load_irb_text(pdf_path: str) -> str:
    """Extract raw text from the IRB PDF (page-by-page concatenation)."""
    reader = PdfReader(pdf_path)
    return "\n".join([p.extract_text() or "" for p in reader.pages])

def extract_irb_scope(text: str) -> Dict[str, Any]:
    """
    Parse key IRB fields from the text using regex (population, timeframe, sites, etc.).
    Also normalizes lists (sites, approved elements) for easier downstream checks.
    """
    scope = {}
...
def get_embeddings(backend: str):
    """
    Return an embeddings instance that MUST MATCH the model used to build FAISS.
    backend: "openai" (cloud) or "local" (HuggingFace sentence-transformers).
    """
    if backend == "openai":
        ...
        # Sentence-transformers local model (offline-friendly)
...
def load_store(path: str, embeddings):
    """Load a persisted FAISS index from disk with the given embeddings object."""
...
def build_retrievers(db_root: str, embeddings, k: int = 4) -> Dict[str, Any]:
    """
    Build a retriever per corpus (HIPAA/GDPR/IRB) from ./vectordb/{hipaa|gdpr|irb}.
    The retrievers expose top-k semantic search to pull short, grounded excerpts.
    """
    paths = {
...
def evaluate_rules(irb_scope: Dict[str,Any], req: Dict[str,Any]) -> List[Dict[str,Any]]:
    """
    Core rule engine:
    - Aligns request with IRB scope (sites, population, timeframe).
    - Flags HIPAA minimum-necessary and de-ID/LDS requirements.
    - Checks HIPAA Authorization / IRB Waiver when identifiers are present.
    - Adds GDPR lawful basis checks if EU jurisdiction is in scope.
    Returns a list of finding dicts with pass/issue, citations, and remediation.
    """
    results = []

    # IRB scope alignment (sites/population/time window)
    ...
    if "timeframe" in irb_scope and "time_window" in req:
        ...
        # naive parse: infer IRB start if present (YYYY → YYYY-01-01)
        irb_start = ""
    ...
    # HIPAA minimum necessary & De-ID/Limited Dataset checks based on requested elements
    requested = set(e.lower() for e in req.get("data_elements",[]))
    direct = [e for e in requested if e in DIRECT_IDENTIFIERS]
    if direct:
        # Direct identifiers present → minimum necessary fails; suggest De-ID or LDS + DUA
        results.append({
        ...
        # No direct identifiers → these two checks pass
        results.append({
            "rule_set":"hipaa","id":"hipaa_minimum_necessary","title":"HIPAA Minimum Necessary",
    ...
    # HIPAA Authorization or IRB Waiver requirement when identifiers are present
    ...
    # GDPR checks only if EU jurisdiction is included in the request
...
def decide_verdict(results: List[Dict[str,Any]]) -> str:
    ...
    - Any IRB failure → Not allowed.
    - Otherwise, any HIPAA failure → Allowed with conditions.
    - Else → Allowed.
    """
...
def collect_policy_excerpts(retrievers: Dict[str,Any], queries: Dict[str, List[str]] = None) -> str:
    """
    Retrieve short, top-1 excerpts per query from each corpus to ground the report with citations.
    Falls back gracefully if a retriever errors.
    """
...
def render_report_markdown(verdict: str, results: List[Dict[str,Any]], irb_scope: Dict[str,Any],
    ...
    """
    Draft a final compliance report:
    - If OpenAI chat is available, generate a polished Markdown with bullets and anchors.
    - Else, produce a deterministic Markdown summary (no external calls).
    """
...
def main():
    ...
    1) Parse CLI args and load IRB PDF + request JSON.
    2) Load embeddings (match FAISS build).
    3) Create retrievers for HIPAA/GDPR/IRB.
    4) Evaluate rules and decide a verdict.
    5) Retrieve short policy excerpts for grounding.
    6) Render a Markdown report (LLM if available, else deterministic).
    7) Write JSON output with verdict, findings, scope, and report.
    """
